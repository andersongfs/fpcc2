...
### gasto mensal com combustivel e lubrificantes
```{r}
ano_atual = 2017
...
top_n(arrecadacao_postos, 20, total_arrecadado)
top20 = top_n(arrecadacao_postos, 20, total_arrecadado)

ggplot(top20, aes(x=reorder(txtFornecedor, total_arrecadado), y=total_arrecadado/1000)) + 
  geom_bar(stat = "identity") +
...
```{r}
top10 = c("QUADRAMARES COMÉRCIO DE COMBUSTÍVEIS LTDA", "MÁRCIA MOURA RAMADAN - POSTO SÃO MARCOS")
total_gasto_relacionamento = gastos_gerais %>%
  filter(txtDescricao == "COMBUSTÍVEIS E LUBRIFICANTES.", sgUF == "PB", numAno < ano_atual) %>%
  group_by(txtCNPJCPF, txNomeParlamentar) %>%
  summarise(total_arrecadado = sum(vlrLiquido) )

nome_fornecedor = gastos_gerais %>%
  filter(txtDescricao == "COMBUSTÍVEIS E LUBRIFICANTES.", sgUF == "PB", numAno < ano_atual) %>%
  select(txtCNPJCPF, txtFornecedor) %>%
  distinct(txtCNPJCPF, .keep_all = TRUE)

relacionamento_merge = merge(total_gasto_relacionamento, nome_fornecedor, by = "txtCNPJCPF")

numero_de_relacionamentos = relacionamento_merge %>%
  group_by(txtFornecedor)%>%
  summarise(relacionamentos = n())

numero_de_relacionamentos_ordenado = numero_de_relacionamentos[order(-numero_de_relacionamentos$relacionamentos), ]
numero_de_relacionamentos_top = top_n(numero_de_relacionamentos_ordenado, 10, numero_de_relacionamentos_ordenado$relacionamentos) #n sei pq ta pegando 24

ggplot(numero_de_relacionamentos_top, aes(x=reorder(txtFornecedor, relacionamentos), y=relacionamentos)) + 
  geom_bar(stat = "identity") +
